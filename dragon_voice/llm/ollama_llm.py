...
class OllamaBackend(LLMBackend):
    ...
    def capabilities(self) -> frozenset[Modality]:
        """Modalities for the configured ollama model.

        Delegates to the centralized capability registry (#200, Wave 21).
        See `dragon_voice/llm/capability_registry.py:detect_ollama` for
        the substring-based detection logic (vision hints, audio for
        minicpm-o, tool-calling for the curated FC family list).
        """
        from .capability_registry import detect

        return detect("ollama", self._model)
