...
## Discovered-during-execution items

- [x] **W15-C06** `[TT]` PSRAM task stacks + NULL guards — internal-SRAM starvation was causing first-boot Settings/Chat panics. `tab5_worker` + `ui_task` moved to PSRAM (+48 KB reclaimed), NULL guards on every `lv_*_create` in `mk_section`/`mk_row_label`/`mk_row_value`/`mk_slider`/`mk_switch`/`mk_pill_btn`/`row_create` + upfront heap gate at top of `ui_settings_create()`. heap_wd DMA-reboot path disabled (was firing on steady-state draw). TT #98 merged.
- [x] **W15-C07** `[TT]` Voice overlay fade-in rasterizer crash — tap orb with chart widget active → `lv_draw_sw_fill → lv_memset(dst=?, v=255, len=640)` panic during 200 ms fade-in composite. Fix: snap to opaque (matches pre-existing fade-out fix). Verified 6 rapid orb-open/close cycles with widgets, 0 reboots. TT #98 merged.
- [ ] **W15-C08** `[TT]` Tab5 WiFi radio drops association under sustained HTTP/WS hammer load — Tab5's own serial log shows `ui_task alive, lvgl_fps=13` but ARP/ICMP stops responding from the workstation for ~30-60 s. Likely WiFi RX/TX descriptor pool starvation. Triggers the "Dragon Unreachable" hero if the WS disconnect lasts >20 s. Follow-up investigation; not blocking normal user flow.
- [x] **W15-OPS** `[TB]` `dragon_voice/config.yaml` `ollama_model: qwen3:1.7b` → `qwen3:0.6b` — the 1.7b model was never installed on Dragon (only 0.6b/4b/nemotron/ministral are); every LOCAL turn hung 30 s while Ollama tried to pull. TB #54 merged.
- [x] **W15-OPS-2** `[TB]` `dragon_voice/config.yaml` `ollama_model: qwen3:0.6b` → `ministral-3:3b` (2026-04-24) — re-benchmarked all 11 sub-4B models on Dragon after TinkerTab audit revealed 0.6b's raw-`<tool>` XML leaks and silent fails on tool-forcing prompts.  ministral-3:3b was the only sub-4B model that (a) fit Tab5's ~30 s WS keepalive, (b) did non-trivial math correctly (456×789=359,784), and (c) stored a memory fact without leaking prior-session context.  CLAUDE.md Local LLM Benchmarks table rewritten with all 11 results + three failure classes (too-small / too-slow / fluent-hallucinator).  See `docs/historical/AUDIT-WAVE-14.md` "Local-mode model gauntlet 2026-04-24" section.
