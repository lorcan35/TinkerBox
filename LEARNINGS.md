...
## W7-F Wave 7 — OpenClaw gateway handshake required 4 schema fixes uncovered only by live test

- **Symptom (W7-F.2 → W7-F.3 chain):** Initial `GatewayConnector` was modelled on the OpenClaw protocol docs but the live gateway rejected the connect frame with `ack_fail "gateway connect rejected: invalid connect param"`.  Each fix surfaced the NEXT validation failure in a chain of 4.
- **Root causes (each a separate sub-PR):**
  1. **`client.id` must be a `GATEWAY_CLIENT_IDS` enum member.**  We used `tinkerbox-dragon` (descriptive); the gateway enforces `gateway-client` / `agent-runner` / etc.  Free-form ID names are rejected even when otherwise well-formed.
  2. **`role` must be `operator` or `node`** per `parseGatewayRole()`.  We used `agent.runner` (matches the JS gateway's *handler* naming); the connect-time role is the broader scope category, not the named handler.
  3. **`operator.admin` does NOT transitively grant `operator.write`.**  Method scopes are checked individually per `method-scopes.ts::WRITE_SCOPE`.  The fix: explicitly include `operator.read` + `operator.write` alongside `operator.admin` in the scope list.
  4. **`PROTOCOL_VERSION` is a single integer (3 today), not a string or range.**  We initially sent `"1.0"`.  Quoted a forward-compatible window `minProtocol=1, maxProtocol=3` so a server bump within range doesn't require connector redeploy.
- **Why none caught in tests:** The MockConnector + FakeGateway harness validated frame *shape* but not enum values — the gateway's runtime enum check fires AFTER shape validation passes.  Lesson: any external-service connector test needs to validate against the same enum / scope tables the real service uses.
- **Fix (PR #312, TB W7-F.3):** All four params updated; `ChannelGatewayConfig.client_id` default switched to `gateway-client` so fresh deploys work without yaml override.  Live verification: pre-fix ack was the schema rejection; post-fix the ACK changed to `missing scope: operator.write` — handshake succeeded, only the next layer's authorization check blocked.  That's progress measured one error message at a time.
- **Prevention:** When integrating with an external WS-RPC gateway:
  1. Read the gateway's runtime validation tables (`GATEWAY_CLIENT_IDS`, `parseGatewayRole`, `method-scopes.ts`) BEFORE writing the connector — not after the first ack_fail.
  2. Mirror those tables in your test fixture so MockConnector rejects invalid params identically to the real gateway.
  3. Track each `ack_fail` reason as a separate test case — they form a contract chain, not noise.

## W7-F.4 Wave 7 — Backend self-pairing requires ed25519 device identity + connect.challenge nonce echo

- **Why W7-F.3 still wasn't enough:** Even with all 4 schema params correct, the gateway response was `missing scope: operator.write`.  Reason: OpenClaw's `shouldSkipBackendSelfPairing` helper requires the connector to authenticate as a *backend* device with a real ed25519 keypair — not just a token-bearing client.  Without device identity, the backend can't auto-grant write scopes to itself.
- **Three pieces needed:**
  1. **ed25519 keypair persistence.**  `dragon_voice/channels/device_identity.py` (~240 LOC) generates + persists keypair to `~/.dragon/identity/device.json` (0o600 file mode, atomic temp-write+rename, regenerates on corruption).  Public key + signature are base64url-no-padding (matches OpenClaw's encoding exactly).
  2. **Canonical payload string.**  `build_v3_payload()` builds the pipe-delimited canonical string `clientId|deviceId|nonce|timestamp|protocol|role|scopes|metadata` byte-for-byte matching `buildDeviceAuthPayloadV3()` in OpenClaw's `device-identity.ts`.  Scopes joined with `,`; metadata lowercase-ASCII; None token serializes as empty string.  Any drift here = invalid signature = rejected handshake.
  3. **Challenge-nonce flow.**  Gateway sends `event: connect.challenge` immediately after WS open.  Connector must prepare an `asyncio.Future` BEFORE the WS open (or the reader will race the handshake), the reader dispatches the challenge event into the future, `_handshake` waits on it (bounded), signs `build_v3_payload(...)` with the device's private key, includes `device: {id, publicKey, signature, signedAt, nonce}` in the connect frame.
- **Test contract:** FakeGateway harness sends `connect.challenge` first and asserts the connector echoes the server nonce — locks the contract.  Without this, any future change that "fixes" the connector could silently break against the real gateway by accepting any nonce.
- **CI gate:** `cryptography>=42.0.0` added to `requirements.txt` (CI runner is minimal — the standard library doesn't ship ed25519).
- **Prevention:** For WS-RPC connectors with cryptographic handshakes, locate the **canonical-string builder** in the server's source and **byte-for-byte mirror it** in your client.  Don't infer the format from docs — the docs probably describe an earlier version.  Lock the contract with a fixture test that asserts the canonical string hashes match a known value.

## W7-F.5 Wave 7 — Channel name resolution: Tab5 short ids ≠ OpenClaw plugin canonical names

- **Symptom:** Post-W7-F.4 the connect succeeded, scopes were granted, `send` was invoked — but the gateway responded `unsupported channel: tg` even though Telegram was configured + enabled on Dragon.
- **Root cause:** Tab5 uses 2-char canonical channel codes (`tg`, `wa`, `dc`, `sl`, `sg`, `im`, `ma`, `em`) because the firmware's NVS key length cap is 15 chars (`ch_tg_on` fits; `ch_telegram_on` would too but `ch_whatsapp_on` is right at the boundary).  OpenClaw's channel plugin registry resolves by full canonical plugin id (`telegram`, `whatsapp`, …) — there are no two-letter aliases.  Tab5's `tg` got forwarded verbatim and the registry's lookup failed.
- **Fix (PR #314, TB W7-F.5):** New `_TAB5_CHANNEL_ALIASES` dict in `dragon_voice/channels/gateway.py` covers all 8 Tab5 channels.  `send_reply` consults it via `dict.get(channel, channel)` so canonical names + future plugin ids fall through unchanged.  Pre-fix ack: `unsupported channel: tg`; post-fix ack: `Error: Telegram send failed: chat not found (chat_id=42)` — connector reaches the real Telegram API end-to-end.  A real paired bot will round-trip to `ok=true` with a real `platform_message_id`.
- **Prevention:** Whenever two systems use the same vocabulary domain (channel names, model ids, user roles) but have different encoding constraints (NVS key length, URL slug rules, JSON-safe characters), assume their canonical forms will diverge.  Build the mapping at the integration boundary, not at either endpoint.  Verify the mapping with a test that loops over the full vocabulary (W7-F.5 added `TestTab5ChannelAliases::test_all_8_covered` for exactly this).

## W7-E (Tab5) ↔ W7-F (Dragon) — Reply round-trip closure required Dragon-side stub before real gateway

- **Symptom:** Tab5 W7-E.4b shipped real voice-dictated `channel_reply` frames (TT #471) but Dragon was logging `Unknown command from ws*: channel_reply` and dropping them.  The Tab5 toast "Replied via X" only appeared when a developer manually fired a synthetic ACK via the Tab5 debug-inject endpoint.
- **Decision:** Ship a Dragon-side ACK STUB before the real `GatewayConnector` was ready.  The stub (TB #305) immediately ACKs every `channel_reply` with `{ok: true, platform_message_id: "stub:<hex>"}`.  Real platform forwarding (W7-F.2 onwards) layers on top — the Tab5 surface doesn't change.
- **Why this was right:** Decouples the cross-stack UX validation (Tab5 toasts firing on real WS round-trip) from the real platform integration (which has its own multi-step setup — see W7-F.3/4/5).  The Tab5 e2e harness could complete the full notification flow even before any real platform was paired.
- **Prevention:** When a cross-stack feature has a "send → ack → display" loop, ship the ack stub on the receiving side as soon as the wire shape is locked.  Don't wait for full implementation — the stub closes the loop for UX testing and locks the contract.  Real implementation then plugs in below the ack layer.

## Architecture decision 2026-05-16 — TinkerBox builds its own integrations layer; OpenClaw becomes optional, not gating

- **Context:** Until now the agentic capabilities of the product (multi-step planning, MCP tools, browser automation, the OpenClaw skill catalog) only fire in `vmode=3` (TinkerClaw mode), which routes through OpenClaw's `/v1/chat/completions` agent runtime.  Modes 0/1/2 (Local / Hybrid / Cloud) have a small built-in `ToolRegistry` (10 tools) but no integrations with Google services, smart home, music, calendar, etc.  Net effect: if a user wants real-world agentic UX, they're locked into `vmode=3` — which makes the whole product feel like "OpenClaw with a Tab5 frontend."
- **Decision:** Build a first-class **Integrations layer** inside TinkerBox at `dragon_voice/tools/integrations/` (modeled on the TTS registry pattern from #339).  Each integration owns its OAuth lifecycle (default: device-code grant per RFC 8628 — works for headless boxes), credential storage (`~/.tinkerclaw/integrations/{name}.json` mode `0o600`), and exposes a set of Dragon `Tool` instances that work in ALL vmodes.  vmode=3 still works through OpenClaw; we additionally expose TinkerBox's integration tools to OpenClaw via a small local MCP server (`tinkerbox-mcp`) so the agent runtime can call them by name.
- **Default policy for new features:** TinkerBox-native by default.  OpenClaw piggyback only when the feature has heavy multi-tenant OAuth complexity (Gmail, Calendar, Notion still TinkerBox-native because user explicitly asked for those; but e.g. "browser automation for arbitrary websites" stays OpenClaw because Playwright lives there).
- **"Steal ideas from OpenClaw" policy:** OpenClaw is MIT-licensed.  When implementing an integration that has an existing OpenClaw MCP, read the upstream impl, understand the API surface choices + OAuth scopes, then reimplement in Python on Dragon side.  Cite inspiration in commit message ("Pattern adapted from openclaw/mcp-server-gmail").  Don't 1:1 port — adapt to our tool patterns and Python idioms.
- **OAuth choice — device-code grant:**  Dragon is headless.  Standard "redirect back to localhost:PORT" doesn't work.  Device authorization grant (RFC 8628) is supported by Google + Spotify natively and works perfectly for headless boxes: user gets a code, types it on their phone, Dragon polls the token endpoint.  No callback server needed.  For providers that only support PKCE (Notion, Linear) we'll handle them per-integration when we get there.
- **Cross-vmode tool exposure:** All integration tools register in `ToolRegistry`, so:
  - vmode=0/1/2 — Dragon's `ConversationEngine` formats the tool catalog for the LLM, the LLM emits tool-call markers, Dragon executes.
  - vmode=3 — `tinkerbox-mcp` server inside Dragon at `localhost:3503` exposes the same tools.  OpenClaw's `mcp.servers` config consumes it.  OpenClaw agents see TinkerBox tools alongside OpenClaw's own MCPs.
- **OpenClaw position after this lands:** OpenClaw becomes ONE OF THREE agent backends, not THE agentic backend:
  - vmode=0/1: TinkerBox-native + local LLM (now actually agentic because of integrations).
  - vmode=2: TinkerBox-native + cloud LLM (probably the best agentic experience — Claude/GPT + our tools).
