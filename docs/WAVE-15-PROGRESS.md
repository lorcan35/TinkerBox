...
- [x] **W15-OPS-2** `[TB]` `dragon_voice/config.yaml` `ollama_model: qwen3:0.6b` → `ministral-3:3b` (2026-04-24) — re-benchmarked all 11 sub-4B models on Dragon after TinkerTab audit revealed 0.6b's raw-`<tool>` XML leaks and silent fails on tool-forcing prompts.  ministral-3:3b was the only sub-4B model that (a) fit Tab5's ~30 s WS keepalive, (b) did non-trivial math correctly (456×789=359,784), and (c) stored a memory fact without leaking prior-session context.  CLAUDE.md Local LLM Benchmarks table rewritten with all 11 results + three failure classes (too-small / too-slow / fluent-hallucinator).  See `docs/AUDIT.md` "Local-mode model gauntlet 2026-04-24" section.
